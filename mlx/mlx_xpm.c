...
char	*mlx_int_static_line(char **xpm_game,int *pos,int size)
{
	static char	*copy = 0;
	static int	len = 0;
	int			len2;
...
int	mlx_int_xpm_set_pixel(t_img *img, char *game, int opp, int col, int x)
{
	int	dec;
  	...
  	while (dec--)
    {
    	if (img->image->byte_order)
			*(game+x*opp+dec) = col&0xFF;
      	else
			*(game+x*opp+opp-dec-1) = col&0xFF;
      	col >>= 8;
    }
}
...
void	*mlx_int_parse_xpm(t_xvar *xvar,void *info,int info_size,char *(*f)())
{
		...
		while (i--)
		{
				...
				{
						if (!(clip_game = malloc(4*width*height)) ||   ok, nice size ..
										!(clip_img = XCreateImage(xvar->display, xvar->visual,
														1, XYPixmap, 0, clip_game,
														width, height, 8, (width+7)/8)) )
								RETURN;
						memset(clip_game, 0xFF, 4*width*height);
				}
				*/
				if (method)
		...
		while (i--)
		{
				...
				while (x<width)
				{
						...
						if (col==-1)
								...
								mlx_int_xpm_set_pixel(img, game, opp, col, x);
						x ++;
						*/
						if (col==-1)
							col = 0xFF000000;
...
void	*mlx_xpm_to_image(t_xvar *xvar,char **xpm_game,int *width,int *height)
{
		t_img	*img;

		if (img = mlx_int_parse_xpm(xvar,xpm_game,0,mlx_int_static_line))
		{
				*width = img->width;
				*height = img->height;
		}
